...
# @Last Modified time: 2025-09-25 16:53:07
...
def draw_cloud_profile(ax, lat, base, top, cloud_type, hgt, cax=None):
    """
    ...
    cloud_type_mesh = np.full((cloud_type.shape[0], hgt.shape[0]), np.nan)
    ...
    for nray in range(cloud_type.shape[0]):  # nray
        for ncloud in range(cloud_type.shape[1]):  # ncloud
            if ~np.isnan(cloud_type[nray, ncloud]):
                ...
                height_indices = (base_hgt <= hgt[:]) & (hgt[:] <= top_hgt)
                cloud_type_mesh[nray, height_indices] = cloud_type[nray, ncloud]

    # Plot the cloud types
    im = ax.pcolormesh(lat, hgt, cloud_type_mesh.T, cmap=cmap, norm=norm)

    # Set plot limits and labels
    ax.set_ylim(0, 20)  # Assuming height is in km and max height is 20 km
...
def read_CloudSat_data(file_path):
    ...
    try:
        if "CLDCLASS" in file_path:
            ...
            data["start_time"], data["end_time"] = f.read_time(datetime=True)[
                [0, -1]
            ]
            ...
            data["cf"] = np.where(data["cf"] >= 100, np.nan, data["cf"])
...
def filter_and_plot_data(overall_data, time_range_custum):
    ...
    draw_elevation(ax2, time_filtered, elv / 1000)  # Convert elevation to km
    ax2.set_title("Cloud Types", fontsize=14)
    draw_elevation(ax3, time_filtered, elv / 1000)  # Convert elevation to km
    ax3.set_title("Cloud Faction", fontsize=14)
    draw_elevation(ax4, time_filtered, elv / 1000)  # Convert elevation to km
    ax4.set_title("Retreived Effective Radius", fontsize=14)
    draw_elevation(ax5, time_filtered, elv / 1000)  # Convert elevation to km
    ax5.set_title("Retreived IWC", fontsize=14)

    # Save the figure
    start_str_date = overall_data["start_time"].strftime("%Y_%m_%d_%H_%M_%S")
    fig.savefig(
...
def plot_multi_CloudSat_data(base_paths, file_idx, time_range_custum=5000):
    """
